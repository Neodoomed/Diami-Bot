...
export default class EconomyWithdraw extends SubCommand {
    ...
    async Execute(int: ChatInputCommandInteraction) {
        ...
        if (!amount) amount = bal.bank;

        if (amount && bal.bank < amount) {
            embed.setDescription(`No tienes suficientes Créditos en el Banco.`);
            int.reply({
                embeds: [embed],
            });
            return;
        ...
        await EconomySystem.withdrawMoney(
            int.user.id,
            int.guild.id,
            amount || 'all'
        );

        embed.setDescription(`Retiras ${amount} créditos.`).setFields(
            {
